...
public class Tutorial02 {
    public static void main(String[] args) {
        ...
        */
        ...
        try {
            ...
            String[] options = {"Nowa gra", "Opcje", "Wyjście"};
            int selectedOption = 0;

            while (true) {
                drawMenu(terminal, options, selectedOption);
                ...
                if (key != null) {
                    switch (key.getKeyType()) {
                        ...
                        case ArrowDown:
                            ...
                            break;
                        case Enter:
                            handleOptionSelection(options[selectedOption]);
                            break;
                        case EOF:
                            return;
                    }
    ...
    private static void drawMenu(Terminal terminal, String[] options, int selectedOption) throws IOException {
        terminal.clearScreen();
        TerminalSize terminalSize = terminal.getTerminalSize();
        int centerX = terminalSize.getColumns() / 2;
        TextColor textColor = new TextColor.RGB(255,255,255);
        for (int i = 0; i < options.length; i++) {
            String option = options[i];
            int y = terminalSize.getRows() / 2 - options.length / 2 + i;
    ...
    private static void handleOptionSelection(String option) {
        if (option.equals("Nowa gra")) {
            // Tutaj możesz umieścić kod uruchamiający nową grę
        } else if (option.equals("Opcje")) {
            // Tutaj możesz umieścić kod przechodzenia do opcji
        } else if (option.equals("Wyjście")) {
            System.exit(0);
        }
    }
}